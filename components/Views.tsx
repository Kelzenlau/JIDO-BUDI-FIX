...
export const Hero = ({ onPlay }: { onPlay: () => void }) => {
  ...
  // Fix: Typed state with HomeConfig to ensure heroImage property is recognized
...
export const AdsSection = () => {
    // Fix: Typed state with AdsConfig to resolve unintentional comparison error between 'image' and 'video' types
        ...
        <div className="max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-8">
                {team.map((member, index) => (
                    <div key={index} className="bg-white p-8 rounded-[2.5rem] shadow-xl border border-slate-100 flex flex-col items-center text-center hover:-translate-y-4 transition-all duration-500 relative group">
                        ...
                        <div className="w-32 h-32 rounded-full overflow-hidden mb-6 bg-slate-100 border-4 border-white shadow-2xl relative z-10 group-hover:scale-110 transition-transform">
                            <img 
                                src={member.photo || `https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(member.name)}&topType=${member.name === 'Premi' ? 'longHair' : 'shortHair'}&facialHairProbability=0`} 
                                alt={member.name} 
        ...
        try {
            ...
            if(codeSnap.exists() && codeSnap.data().active && codeSnap.data().used < codeSnap.data().limit) {
                ...
                await addDoc(collection(db, 'artifacts', appId, 'users', user.uid, 'vouchers'), {
                    // Fix: Changed score to 0 to ensure numeric consistency for sorting/database requirements
                ...
                // Fix: Cast current usage and increment explicitly to avoid type errors
